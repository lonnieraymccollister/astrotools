...
def binimg():
  ...
  def patch_wcs_for_siril(orig_header, bin_factor):
      # Build a WCS object from the old header
      w = WCS(orig_header)

      # Recompute CRPIX and CDELT for the binning
      orig_crpix = w.wcs.crpix.copy()
      orig_cdelt = w.wcs.cdelt.copy()
      new_crpix  = (orig_crpix - 0.5) / bin_factor + 0.5
      new_cdelt  = orig_cdelt * bin_factor
      ...
      w.wcs.crpix = new_crpix
      w.wcs.cdelt = new_cdelt
      w.wcs.crota = [0.0, 0.0]  # ensure CROTA2 is defined

      # Extract the CD matrix directly
      try:
          cd = w.wcs.get_cd()
      except AttributeError:
          # For older astropy versions:
          # CD = CDELT * PC matrix
          cd = (w.wcs.pc * w.wcs.cdelt[:, None])

      # Start building the new header
      hdr = orig_header.copy()

      # Inject the CD-matrix
      hdr['CD1_1'] = (cd[0, 0], 'deg / pixel')
      hdr['CD1_2'] = (cd[0, 1], 'deg / pixel')
      hdr['CD2_1'] = (cd[1, 0], 'deg / pixel')
      hdr['CD2_2'] = (cd[1, 1], 'deg / pixel')

      # Also inject legacy-style keywords for Siril
      hdr['CDELT1'] = (new_cdelt[0], 'deg / pixel')
      hdr['CDELT2'] = (new_cdelt[1], 'deg / pixel')
      hdr['CROTA2'] = (0.0,          'deg')
      hdr['CUNIT1'] = ('deg',        'units of CRVAL/CDELT')
      hdr['CUNIT2'] = ('deg',        'units of CRVAL/CDELT')

      # Preserve the rest of your WCS keywords
      for key in ('CRVAL1','CRVAL2','CTYPE1','CTYPE2','RADESYS','EQUINOX'):
          if key not in hdr:
              hdr[key] = orig_header.get(key)
